...
def tune():
    ...
    # ── Determine which model won Task 1 ──────────────────────────────────────
    ...
    # ── Parent run wraps all child trials ─────────────────────────────────────
    with mlflow.start_run(run_name=PARENT_NAME) as parent_run:
        ...
        for i, params in enumerate(combos, 1):
            with mlflow.start_run(run_name=f"trial_{i}", nested=True):
                ...
                # Full train on train split → evaluate on test split
                ...
                print(f"  Trial {i:>2}/{total_trials} | params={params} | "
                      f"cv_mae={cv_mae:.3f} | test_rmse={rmse:.3f}")
    ...
    # ── Retrain with best params on full training data & save ─────────────────
    best_model = ModelClass(random_state=42, **best_params)
    best_model.fit(X_train, y_train)
    joblib.dump(best_model, os.path.join(MODEL_DIR, "tuned_model.pkl"))
    # Also overwrite champion for downstream tasks
